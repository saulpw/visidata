...
### Path Options vs Sheet Options

Options that describe a property of the file (like `filetype`) belong on the path, not the sheet. Read via `self.source.options.filetype`, not `self.options.filetype`.
