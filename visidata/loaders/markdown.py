...
def save_md(vs, fn):
    ...
    with open(fn, 'w', encoding=options.encoding, errors=options.encoding_errors) as fp:
        fp.write('|' + '|'.join('%-*s' % (col.width, markdown_escape(col.name)) for col in vs.visibleCols) + '|\n')
        fp.write('|' + '+'.join(markdown_colhdr(col) for col in vs.visibleCols) + '|\n')

        for row in Progress(vs.rows):
            fp.write('|' + '|'.join('%-*s' % (col.width, markdown_escape(col.getDisplayValue(row))) for col in vs.visibleCols) + '|\n')
