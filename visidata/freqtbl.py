...
class HistogramColumn(Column):
    ...
    def calcValue(col, row):
        ...
        typed_val = col.sourceCol.getTypedValue(row)
        if typed_val is None or not col.largest:
            return ""

        filled = histolen * typed_val / col.largest
        filled = max(0, min(histolen, int(round(filled))))
        if filled <= 0:
            return ""

        return histogram * filled
...
"""
    ...
    def selectRow(self, row):
        ...
        self.source.select(
            row.sourcerows, status=False, add_undo=False
    ...
    def resetCols(self):
        ...
        percentCol = Column(
            "percent",
            type=float,
            getter=lambda col, row: len(row.sourcerows) * 100 / col.sheet.source.nRows,
        )

        self.addColumn(countCol)
        self.addColumn(percentCol)
    ...
    def afterLoad(self):
        super().afterLoad()
        if (
            self.nCols > len(self.groupByCols) + 3
        ):  # hide percent/histogram if aggregations added
...
FreqTableSheet.addCommand(
    "",
    "open-preview",
    'vd.push(FreqTablePreviewSheet(sheet.name, "preview", source=sheet, columns=source.columns), pane=2); vd.options.disp_splitwin_pct=50',
    "open split preview of source rows at cursor",
)

Sheet.addCommand(
    "F",
    "freq-col",
    "vd.push(makeFreqTable(sheet, cursorCol))",
    "open Frequency Table grouped on current column, with aggregations of other columns",
)
Sheet.addCommand(
    "gF",
    "freq-keys",
    'vd.push(makeFreqTable(sheet, *keyCols)) if keyCols else vd.fail("there are no key columns to group by")',
    "open Frequency Table grouped by all key columns on source sheet, with aggregations of other columns",
)
Sheet.addCommand(
    "zF",
    "freq-summary",
    'vd.push(makeFreqTableSheetSummary(sheet, Column("Total", sheet=sheet, getter=lambda col, row: "Total")))',
...
ColumnsSheet.addCommand(
    ENTER,
    "freq-row",
    "vd.push(makeFreqTable(source[0], cursorRow))",
...
FreqTableSheet.addCommand(
    ...
    "unselect all source rows grouped in current row",
)
FreqTableSheet.addCommand(
    "g" + ENTER,
    "dive-selected",
    "vd.push(openRows(selectedRows))",
    "open copy of source sheet with rows that are grouped in selected rows",
)
FreqTableSheet.addCommand(
    "",
    "select-first",
    "for r in rows: source.select([r.sourcerows[0]])",
    "select first source row in each bin",
)
