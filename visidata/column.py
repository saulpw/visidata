...
class Column(Extensible):
    ...
    def __init__(self, name=None, *, type=anytype, cache=False, **kwargs):
        self.sheet = ExplodingMock(
            "use addColumn() on all columns"
        )  # owning Sheet, set in .recalc() via Sheet.addColumn
        if name is None:
    ...
    def __copy__(self):
        ...
        if self._cachedValues is not None:
            ...
            )  # an unrelated cache for copied columns
        return ret
    ...
    def __getstate__(self):
        return {
            k: getattr(self, k)
            for k in "name typestr width height expr keycol formatter displayer fmtstr voffset hoffset aggstr".split()
            if hasattr(self, k)
        }
    ...
    def formatted_help(self):
        return MissingAttrFormatter().format(
            self.help, sheet=self.sheet, col=self, vd=vd
        )
    ...
    def make_formatter(self):
        ...
        _formatMaker = getattr(
            self, "formatter_" + (self.formatter or self.sheet.options.disp_formatter)
        )
    ...
    def formatValue(self, typedval, width=None):
        ...
        if self.type is anytype:
            if isinstance(typedval, (dict, list, tuple)):
                ...
                dispval, dispw = clipstr(
                    iterchars(typedval), width - 1
                )  # subtract 1 for left-side margin
                return dispval
    ...
    def displayer_generic(self, dw: DisplayWrapper, width=None):
        ...
        if (
            width is not None
            and width > 1
            and vd.isNumeric(self)
            and isinstance(dw.typedval, (int, float))
        ):
    ...
    def display(self, *args, **kwargs):
        f = getattr(
            self,
            "displayer_" + (self.displayer or self.sheet.options.disp_displayer),
            self.displayer_generic,
        )
    ...
    def getTypedValue(self, row):
        ...
        return wrapply(self.type, wrapply(self._coerceValue, row))

    def _coerceVal(self, val):
        "Pre-coerce val for the column type (e.g. str-to-int via intermediate float)."
        if isinstance(val, str) and self.type is int:
            # int("42.5") fails in Python; coerce through float first
            return float(val)
        return val

    def _coerceValue(self, row):
        "Get raw value and pre-coerce for the column type."
        return self._coerceVal(self.getValue(row))
    ...
    def getCell(self, row):
        ...
        if isinstance(typedval, TypedWrapper):
            if isinstance(cellval, TypedExceptionWrapper):  # calc failed
                ...
                return DisplayWrapper(
                    None,
                    text="",  # force empty display for None
                    note=options.disp_note_none,
                    notecolor="color_note_type",
                )
            elif isinstance(
                typedval, TypedExceptionWrapper
    ...
    def setValuesTyped(self, rows, *values):
        ...
        vd.addUndoSetValues([self], rows)
        for r, v in zip(
            rows, itertools.cycle(self.type(self._coerceVal(val)) for val in values)
        ):
            vd.callNoExceptions(self.setValue, r, v)
...
class SubColumnFunc(Column):
    ...
    def __init__(self, name="", origcol=None, expr=None, subfunc=getitemdef, **kwargs):
        super().__init__(
            name, type=origcol.type, width=origcol.width, expr=expr, **kwargs
        )
