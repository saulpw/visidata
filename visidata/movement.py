...
BaseSheet.addCommand(None, 'mouse-enable', 'mm, _ = curses.mousemask(-1) if hasattr(curses, "mousemask") else (None, None); status("mouse "+("ON" if mm else "OFF"))', 'enable mouse events')
BaseSheet.addCommand(None, 'mouse-disable', 'mm, _ = curses.mousemask(0) if hasattr(curses, "mousemask") else (None, None); status("mouse "+("ON" if mm else "OFF"))', 'disable mouse events')
