...
- [join] add new "concat" jointype to behave similar to "append" but
    keeps first sheet type and columns (requested by @frosencrantz #1598)
...
- [dirsheet] fix case where filenames with a `.` in them had parts
    of the name after the `.` duplicated (reported by @kunliugithub #1571)
...
- [mouse] fix mouse-clicks on statusbar when splitpane is off (reported by @frosencrantz #1625)S
- [mouse] open `sheets-stack` for left on sheet name (reported by @frosencrantz #1625)
...
- [sqlite] add passtrhough options for `connect()` (reported by @cwarden #1622)
